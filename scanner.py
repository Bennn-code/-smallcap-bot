...
def scan_market(settings: Settings, max_alerts: int = 5, debug: bool = False, print_debug: bool = False) -> ScanResult:
    ...
    if print_debug:
        print("DEBUG 掃描摘要")
        print(f"USDT可交易合約數：{stats.bybit_symbols}")
...
def format_daily_summary(result: ScanResult, settings: Settings) -> str:
    lines = [
        "📋 今日小幣雷達摘要",
        "",
        f"USDT合約：{result.stats.bybit_symbols}",
...
def run(settings: Settings, once: bool, max_alerts: int, debug: bool, summary: bool) -> None:
...
def main() -> None:
    ...
    run(get_settings(), once=args.once, max_alerts=args.max_alerts, debug=args.debug, summary=args.summary)
