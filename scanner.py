...
def scan_market(settings: Settings, max_alerts: int = 5, debug: bool = False, print_debug: bool = False) -> ScanResult:
    ...
    stats = ScanStats(
        ...
        early_count=sum(1 for item in candidates if item.level == "EARLY"),
        watch_count=sum(1 for item in candidates if item.level == "WATCH"),
...
def format_watch_summary(result: ScanResult, settings: Settings) -> str:
    lines = [
        "🔎 小幣雷達觀察摘要（4小時）",
        "",
        "這是觀察名單，用來看接近條件但還需要確認的幣。",
        "",
        "📌 市場狀態",
        f"合約 {result.stats.bybit_symbols}｜CMC {result.stats.cmc_rows}｜初篩 {result.stats.prelim_count}",
        f"HIGH {result.stats.high_count}｜EARLY {result.stats.early_count}｜WATCH {result.stats.watch_count}",
        f"OI資料 {result.stats.oi_ok_count}｜有方向 {result.stats.directional_count}",
        "",
        "🔥 最接近標的",
    ]
    ...
    for index, candidate in enumerate(result.candidates[:max(1, min(10, len(result.candidates)))], start=1):
        ...
        lines.extend(
            [
                "",
                f"{index}. {candidate.symbol}｜{candidate.level}｜{display_direction(candidate.direction)}",
                f"   分數 {score_bar(candidate.score)}",
                f"   1h {candidate.pct_1h:+.1f}%｜24h {candidate.pct_24h:+.1f}%｜7d {candidate.pct_7d:+.1f}%",
                f"   市值 {candidate.market_cap_bucket}｜OI1h {oi_1h}｜OI4h {oi_4h}",
                f"   缺口：{gaps}",
            ]
        )
...
def candidate_gaps(candidate: Candidate) -> str:
    gaps: list[str] = []
...
def format_test_message(settings: Settings) -> str:
    return "\n".join(
        [
            ...
            "",
            "新版規則已載入：",
            f"• HIGH：{settings.high_alert_score}分以上正式推薦，不受每日5則限制",
            f"• EARLY：{settings.early_alert_score}-74分，掃到就即時推，每天最多 {settings.early_daily_limit} 則",
            f"• 同幣同方向同等級 cooldown：{settings.cooldown_hours} 小時",
            "• EARLY 升 HIGH 可以再次推送",
            "• 70-74分即使方向待觀察，也會即時推送",
            "• 觀察名單：每4小時推一次",
            f"• U型底：三天掃一次，最多 {settings.u_bottom_limit} 則；沒有候選也會通知",
            "",
            "版面預覽：",
            "🟡🌱 小幣雷達 | EARLY | 待觀察",
            "分數：███████░░░ 72/100",
            "🧭 方向理由 / 📊 價格動能 / 📈 合約量能 / 下一步 會分段顯示",
        ]
    )
...
def run(
    ...
    while True:
        ...
        if u_bottom:
            ...
            sent_count = 0
            if not result.candidates:
                message = f"🟣🥣 U型底反轉雷達\n\n本次已掃描 {result.scanned} 個標的，目前沒有符合 U型底反轉條件的幣。"
                if settings.dry_run:
                    print(message)
                else:
                    send_telegram_message(
...
def main() -> None:
    ...
    parser.add_argument("--max-alerts", type=int, default=50, help="Max candidates per scan")
